...
public class ChatIndexAdapter extends RecyclerView.Adapter<ChatIndexAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        ...
        Realm.getInstance(RealmUtility.getDefaultConfig(mActivity)).executeTransaction(realm -> {
            RealmWholesalerCart realmStockCart = Realm.getInstance(RealmUtility.getDefaultConfig(mActivity))
                    .where(RealmWholesalerCart.class)
                    .equalTo("seller_id", realmChat.getSeller_id())
                    .findFirst();

            holder.name.setText(realmStockCart.getFirst_name());

            if (realmStockCart.getProfile_image_url() != null && !realmStockCart.getProfile_image_url().equals("")) {
                Glide.with(mActivity)
                        .load(realmStockCart.getProfile_image_url()) // image url
